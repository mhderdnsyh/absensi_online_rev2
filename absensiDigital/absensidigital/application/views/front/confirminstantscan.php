            ...
            <div class="card shadow-lg border-0 rounded-lg">
                ...
                <div class="card-body">
                    ...
                    <dl class="row">
                        ...
                        <dd class="col-sm-7" id="nama_pegawai"><?= $cfmabs['namalengkap'] ?></dd>
                        <dt class="col-sm-5">Kode Pegawai:</dt>
                        <dd class="col-sm-7"><?= $cfmabs['kodeGtk'] ?></dd>
                        <dt class="col-sm-5">NPWP:</dt>
                        <dd class="col-sm-7"><?= $cfmabs['nip'] ?></dd>
                        <dt class="col-sm-5">Bagian Shift:</dt>
                        <dd class="col-sm-7"><?= ($cfmabs['bagianShift'] == 1) ? '<span class="badge badge-success">Full Time</span>' : (($cfmabs['bagianShift'] == 2) ? '<span class="badge badge-warning">Part Time</span>' : '<span class="badge badge-primary">Shift Time</span>'); ?></dd>
                    </dl>
                    <hr>
                    ...
                    <div class="text-center" id="func-absensi">
                        <p class="font-weight-bold">Status Kehadiran: <?= $statusGtk = (empty($absensi['statusGtk'])) ? '<span class="badge badge-primary">Belum Absen</span>' : (($absensi['statusGtk'] == 1) ? '<span class="badge badge-success">Sudah Absen</span>' : '<span class="badge badge-danger">Absen Terlambat</span>'); ?></p>
                        <div id="jamabsen" class="d-flex justify-content-between">
                            <p>Waktu Datang: <?= $jamMasuk = (empty($absensi['jamMasuk'])) ? '00:00:00' : $absensi['jamMasuk']; ?></p>
                            <p>Waktu Pulang: <?= $jamMasuk = (empty($absensi['jamPulang'])) ? '00:00:00' : $absensi['jamPulang']; ?></p>
                        </div>
                    </div>
                ...
                <div class="card-footer">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="text-muted">Absen Datang Jam: <?= $dataapp['absenMulai'] ?></div>
                        <div class="text-muted">Absen Pulang Jam: <?= $dataapp['absenPulang']; ?></div>
                    </div>
                </div>
            </div>
        </div>
